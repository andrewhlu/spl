...
uint8_t SPI_READ_MISO(){
	...
	return bitstatus;
	//printf ("%d\r\n",(GPIOA->IDR & GPIO_PIN_6));
}

uint8_t SPI_READ_IRQ(){
	GPIO_PinState bitstatus = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_5);
	return bitstatus;
	//printf ("%d\r\n",(GPIOA->IDR & GPIO_PIN_6));
}
...
void SPI_IoInit(void)
{
  ...
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
	...
	//Configure GPIO pins : PB13, 14,15
...
static bool rfm95_write(rfm95_handle_t *handle, rfm95_register_t reg, uint8_t value)
{
	...
	//	return false;
	//}
	SPI_WriteByte(transmit_buffer[0]);
	SPI_WriteByte(transmit_buffer[1]);
...
static bool rfm95_send_package(rfm95_handle_t *handle, uint8_t *data, size_t length, uint8_t channel)
{
	...
	while (SPI_READ_IRQ() == GPIO_PIN_RESET) {
